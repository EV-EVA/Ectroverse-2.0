...
if( ( (cnt->session)->dbuser ) && ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) {
	...
	httpString( cnt, "&nbsp;-&nbsp;" );
	httpPrintf( cnt, "<a href=\"%s&id=%d\" target=\"main\">Leader Options</a><br><br>", URLAppend( cnt, "ministers" ), curfam );
}
...
empired.numplayers = b;
 ...
 for( a = first ; a != -1 ; )
 {
  ...
  if( ( id >= 0 ) && ( user ) && ( ( curfam == maind.empire ) || (nAlly == CMD_RELATION_ALLY) || ( ( (cnt->session)->dbuser ) && ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) ) )
  {
   ...
   else if( ( empired.politics[CMD_POLITICS_LEADER] == id ) || ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) )
   {
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 if( ( main2d.aidaccess == 0 ) ) {
    ...
    httpString( cnt, "No access<br><br>" );
    continue;
  } else if( ( main2d.aidaccess == 2 ) && !( bitflag( ((cnt->session)->dbuser)->flags, CMD_USER_FLAGS_LEADER ) ) ) {
    httpString( cnt, "No access<br><br>" );
    continue;
  }
...
if( evote ) {
	...
	if( empired.politics[CMD_POLITICS_LEADER] == -1 ) {
		httpString( cnt, "<b>Your empire didn't elect a Leader yet" );
...
if( ( dbEmpireGetInfo( curfam, &empired ) ) < 0 ) {
 ...
 if( ( b = dbEmpireRelsList( curfam, &rel ) ) >= 0 )
 {
  ...
  for( a = 0 ; a < b ; a += 4 )
  {
   if( ( rel[a+3] & 1 ) || ( rel[a+1] != CMD_RELATION_NAP ) )
    continue;
   for( c = 0 ; c < b ; c += 4 )
   {
...
iohtmlBodyInit( cnt, "Empire Politics" );

if( dbEmpireGetInfo( curfam, &empired ) < 0 ) {
	httpString( cnt, "This empire does not seem to exist!" );
	goto RETURN;
}
if( dbEmpireGetMessage( curfam, &message ) < 0 ) {
	httpString( cnt, "Error getting Empire infomation!" );
	goto RETURN;
}
if( ( (cnt->session)->dbuser ) && ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) {
	goto ACCESS;
...
for( politics = 0; politics < CMD_POLITICS_NUMUSED; politics++ ) {
	if( empired.politics[politics] == id ) {
		goto ACCESS;
	}
}

httpString( cnt, "You do not have permission to access this page." );
goto RETURN;
ACCESS:
...
if( hqmesstring ) {
	...
	if( dbEmpireSetMessage( curfam, &message ) < 0 ) {
		httpString( cnt, "<i>Error changing Leader Announcement...</i><br><br>" );
	} else {
		httpString( cnt, "<i>Leader Announcement</i><br><br>" );
	}
...
httpPrintf( cnt, "<tr><td><form enctype=\"multipart/form-data\" action=\"%s\" method=\"POST\">Empire picture</td></tr>", URLAppend( cnt, "ministers" ) );
...
if( ( b = dbEmpireRelsList( curfam, &rel ) ) >= 0 ) {
	...
	for( a = 0 ; a < b ; a += 4 ) {
		...
		for( c = 0 ; c < b ; c += 4 ) {
			if( !( rel[c+3] & 1 ) || ( rel[c+1] != CMD_RELATION_NAP ) || ( rel[a+2] != rel[c+2] ) )
				continue;
			httpPrintf( cnt, "<i>NAP with empire #%d</i><br>", rel[a+2] );
		}
	}
	free( rel );
}
...
httpPrintf( cnt, "<tr><td><form action=\"%s\" method=\"POST\">Leader message</td></tr>", URLAppend( cnt, "ministers" ) );
